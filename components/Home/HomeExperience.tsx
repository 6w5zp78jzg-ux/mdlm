  ...
  return (
    <div style={{ position: "fixed", inset: 0, width: "100%", height: "100vh", overflow: "hidden", background: "#000" }}>
      ...
      <style>{`
        ...
        .anim-top-text { animation: revealTopText 7s cubic-bezier(0.19, 1, 0.22, 1) both; }
        .anim-left-text { animation: slideImpactLeft 7s cubic-bezier(0.16, 1, 0.3, 1) both; }
        .anim-right-text { animation: slideImpactRight 7s cubic-bezier(0.16, 1, 0.3, 1) both; }
        .anim-bottom-text { animation: revealBottomText 7s cubic-bezier(0.25, 1, 0.5, 1) both; }
        ...
        <div className="stage-perspective master-glow" style={{
          ...
          <div key={titleIdx} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <p className="anim-top-text" style={{
              ...
              fontSize: "clamp(0.6rem, 1vw, 0.9rem)", fontWeight: 300,
            ...
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", flexWrap: "wrap", lineHeight: 0.85, padding: "10px 0", gap: "0.15em" }}>
              <span className="anim-left-text" style={{
                fontFamily: "'Helvetica Neue', 'Inter', sans-serif", fontSize: "clamp(3rem, 9vw, 9rem)",
                fontWeight: 100, color: "transparent", WebkitTextStroke: "1px rgba(255, 255, 255, 0.95)",
                letterSpacing: "-0.01em"
              }}>
                {dynamicPhrases[titleIdx].mainLeft}
              </span>
              <span className="anim-right-text" style={{
                fontFamily: "'Playfair Display', serif", fontSize: "clamp(3.5rem, 10vw, 10rem)", 
                fontWeight: 700, fontStyle: "italic", color: "#ffffff", letterSpacing: "-0.04em"
              }}>
                {dynamicPhrases[titleIdx].mainRight}
              </span>
            </div>

            <p className="anim-bottom-text" style={{
              fontFamily: "'Helvetica Neue', sans-serif", fontWeight: 400,
              fontSize: "clamp(0.65rem, 1vw, 0.85rem)", textTransform: "uppercase",
              color: "#c9a96e", margin: "2.5rem 0 0 0", textAlign: "center", letterSpacing: "0.4em"
            }}>
          ...
          <div className="ui-enter" style={{ display: "flex", alignItems: "center", gap: "1.5rem", flexWrap: "wrap", justifyContent: "center", color: "rgba(255,255,255,0.4)", fontFamily: "'Helvetica Neue', sans-serif", fontSize: "clamp(0.45rem, 0.65vw, 0.6rem)", fontWeight: 400, letterSpacing: "0.35em", textTransform: "uppercase", animationDelay: "1.7s" }}>
      ...
      {/* ── PROPIEDADES Z-AXIS ── */}
      <div ref={propertiesRef} style={{ position: "absolute", inset: 0, opacity: 0, pointerEvents: "none", perspective: "1200px", perspectiveOrigin: "center center", background: "radial-gradient(ellipse at center, #0a0a0a 0%, #000000 100%)" }}>
        <div style={{ position: "absolute", inset: 0, transformStyle: "preserve-3d" }}>
          {properties.map((property, i) => (
            <div key={property.id} ref={(el) => { propertyRefs.current[i] = el; }} onClick={() => router.push(`/${locale}/propiedades/${property.slug}`)} style={{ position: "absolute", top: "50%", left: "50%", width: "70vw", height: "75vh", marginLeft: "-35vw", marginTop: "-37.5vh", transformStyle: "preserve-3d", cursor: "pointer", willChange: "transform, opacity, filter" }}>
              <video src={property.video_url} muted playsInline autoPlay loop style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "2px" }} />
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 20%, rgba(0,0,0,0.95) 100%)", pointerEvents: "none" }} />
              <div style={{ position: "absolute", inset: 0, border: "1px solid rgba(255,255,255,0.05)", boxShadow: "inset 0 0 100px rgba(0,0,0,0.5)", pointerEvents: "none" }} />
              
              <div style={{ position: "absolute", top: "-5%", left: "-3%", fontFamily: "'Helvetica Neue', sans-serif", fontSize: "clamp(8rem, 20vw, 16rem)", fontWeight: 100, color: "transparent", WebkitTextStroke: "1px rgba(255, 255, 255, 0.15)", letterSpacing: "-0.05em", lineHeight: 1, pointerEvents: "none", userSelect: "none" }}>0{i + 1}</div>
              <div style={{ position: "absolute", top: "2.5rem", right: "2.5rem", color: "rgba(255,255,255,0.6)", fontFamily: "'Helvetica Neue', sans-serif", fontSize: "0.55rem", fontWeight: 300, letterSpacing: "0.5em" }}>VOL. 0{i + 1}</div>

              <div style={{ position: "absolute", bottom: "3rem", left: "3rem", right: "3rem", display: "flex", flexDirection: "column" }}>
                <p style={{ fontFamily: "'Helvetica Neue', sans-serif", fontSize: "0.55rem", fontWeight: 300, letterSpacing: "0.4em", color: "#c9a96e", textTransform: "uppercase", margin: "0 0 1.2rem" }}>{property.ubicacion}</p>
                <h2 style={{ fontFamily: "'Helvetica Neue', sans-serif", fontSize: "clamp(2rem, 4vw, 3.2rem)", fontWeight: 300, textTransform: "uppercase", color: "#ffffff", letterSpacing: "0.02em", lineHeight: 1.1, margin: "0 0 2rem" }}>{property.titulo[lang]}</h2>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid rgba(255,255,255,0.15)", paddingTop: "1.5rem" }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: "0.5rem" }}>
                    <span style={{ fontFamily: "'Helvetica Neue', sans-serif", fontWeight: 300, fontSize: "1.2rem", color: "#c9a96e" }}>€</span>
                    <p style={{ fontFamily: "'Helvetica Neue', sans-serif", color: "white", fontSize: "clamp(1.5rem, 2.5vw, 2rem)", fontWeight: 200, letterSpacing: "0.05em", margin: 0 }}>{(property.precio / 1000000).toFixed(1)}<span style={{ color: "#c9a96e", fontSize: "0.7em", fontWeight: 300, marginLeft: "0.2rem" }}>M</span></p>
                  </div>
                  <span style={{ fontFamily: "'Helvetica Neue', sans-serif", fontSize: "0.55rem", fontWeight: 400, letterSpacing: "0.4em", color: "#ffffff", textTransform: "uppercase", display: "flex", alignItems: "center", gap: "0.8rem" }}>DISCOVER <span style={{ color: "#c9a96e", fontSize: "1.5em", fontWeight: 300 }}>→</span></span>
                </div>
              </div>
            </div>
          ))}

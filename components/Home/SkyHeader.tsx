...
export default function SkyHeader({ locale }: Props) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (starsRef.current) {
        let v = 0;
        if (tv > 0.5 && tv < 0.95) v = Math.sin(((tv-0.5)/0.45)*Math.PI);
      ...
      <div style={{position:"relative",zIndex:10,display:"flex",flexDirection:"column",alignItems:"center",padding:"0 2rem",userSelect:"none",width:"100%"}}>
        <div key={animKey} style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
          ...
          <div className="tl2" style={{display:"flex",alignItems:"baseline",justifyContent:"center",flexWrap:"wrap",lineHeight:0.85,padding:"10px 0",gap:"0.15em"}}>
            <span style={{fontFamily:"'Helvetica Neue',sans-serif",fontSize:"clamp(3rem,9vw,9rem)",fontWeight:100,color:"transparent",WebkitTextStroke:"1px rgba(255,255,255,0.95)",letterSpacing:"-0.01em"}}>{p?.mainLeft}</span>
            <span style={{fontFamily:"'Playfair Display','Didot',serif",fontSize:"clamp(3.5rem,10vw,10rem)",fontWeight:700,fontStyle:"italic",color:"#fff",letterSpacing:"-0.04em"}}>{p?.mainRight}</span>
          </div>
